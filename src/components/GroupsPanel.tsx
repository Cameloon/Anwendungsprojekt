            ...
            <motion.div key="list" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              ...
              {!rawGroups ? (
                ...
                <div className="space-y-4">
                  ...
                  {archGroups.length > 0 && (
                    <>
                      <p className="text-xs text-muted-foreground flex items-center gap-1"><Archive className="h-3 w-3" /> Archivierte Gruppen</p>
                      <ul className="space-y-2">
                        {archGroups.map((g) => (
                          <li
                            key={g._id}
          ...
          {view === "detail" && active && (
            <motion.div key="detail" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-3">
              <div className="flex gap-1 p-1 rounded-lg bg-secondary/60 w-fit">
                ...
                >
                  <Upload className="h-3.5 w-3.5" /> Dateien
                </button>
                <button
              ...
              {tab === "files" && (
                <div className="space-y-3">
                  <p className="text-xs text-muted-foreground">Dateien in dieser Gruppe</p>
                  <input
                    ref={uploadInputRef}
                    type="file"
                    className="hidden"
                  ...
                  <Button size="sm" variant="outline" className="gap-2" onClick={() => uploadInputRef.current?.click()} disabled={uploading}>
                    <Upload className="h-4 w-4" /> {uploading ? "Lädt..." : "Datei hochladen"}
                  </Button>
                  {groupFiles.length === 0 ? (
